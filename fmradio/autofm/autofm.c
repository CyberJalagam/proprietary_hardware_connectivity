...
static int fm_pmic_read_reg(struct fm_cmd *cmd);
static int fm_pmic_write_reg(struct fm_cmd *cmd);
static int fm_pmic_mod_reg(struct fm_cmd *cmd);
...
static struct fm_cmd g_cmd[] = {
    ...
    {"pmicrd", 1, fm_pmic_read_reg, {NULL, NULL, NULL}, "eg: pmic read 0xa8, desc: read a pmic register"},
    {"pmicwr", 2, fm_pmic_write_reg, {NULL, NULL, NULL}, "eg: pmic write 0xa8,0x00000000, desc: write a pmic register"},
    {"pmicmod", 3, fm_pmic_mod_reg, {NULL, NULL, NULL},
    "eg: mod pmic reg 0xa8,0xfffffffe, 0x00000001, desc: modify a pmic register"},
...
static int fm_pmic_read_reg(struct fm_cmd *cmd) {
    ...
    bzero(&parm, sizeof(struct fm_pmic_rw_parm));
    ...
    ret = ioctl(g_fm_fd, FM_IOCTL_PMIC_RDWR, &parm);
    if (ret) {
        printf("FAIL:%d:%d\n", ret, parm.err);
        return -1;
    }
    printf("OK: 0x%02x = 0x%08x\n", parm.addr, parm.val);
...
static int daemonize(int argc, char **argv)
{
    ...
    fd0 = open(argv[1], O_RDWR | O_CREAT, S_IRUSR | S_IWUSR);
    if (fd0 < 0) {
        printf("FAIL open %s failed %d\n", argv[1], fd0);
    } else {
        close(fd0);
    }
    fd1 = open(argv[2], O_RDWR | O_CREAT, S_IRUSR | S_IWUSR);
    if (fd1 < 0) {
        printf("FAIL open %s failed %d\n", argv[2], fd1);
    } else {
        close(fd1);
    }
    fd2 = open(argv[3], O_RDWR | O_CREAT, S_IRUSR | S_IWUSR);
    if (fd2 < 0) {
        printf("FAIL open %s failed\n %d", argv[3], fd2);
    } else {
        close(fd2);
