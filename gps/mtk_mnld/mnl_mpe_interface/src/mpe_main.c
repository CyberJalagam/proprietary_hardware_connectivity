    ...
    switch (type) {
        ...
        case CMD_SET_GPS_AIDING_REQ: {
            ...
            if(data_file_local != NULL) {
                fprintf(data_file_local,"& %.8lf %.8lf %lf %.8lf %.8lf %lf %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %.3lf %u %f %f %f %f %f\n",
                    mnl_latest_in.latitude[0], mnl_latest_in.longitude[0], mnl_latest_in.altitude[0],
                    ...
                    mnl_latest_in.GNSS_extra_info[3], mnl_latest_in.GNSS_extra_info[4]);
            }
#else
            if (u2Log_enable) {
                snprintf(log_buff, MPE_LOG_BUFF, "& %.8lf %.8lf %lf %.8lf %.8lf %lf %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %.3lf %u %f %f %f %f %f\n",
                    mnl_latest_in.latitude[0], mnl_latest_in.longitude[0], mnl_latest_in.altitude[0],
                    mnl_latest_in.latitude[1], mnl_latest_in.longitude[1], mnl_latest_in.altitude[1],
                    mnl_latest_in.LS_velocity[0], mnl_latest_in.LS_velocity[1], mnl_latest_in.LS_velocity[2],
                    mnl_latest_in.KF_velocity[0], mnl_latest_in.KF_velocity[1], mnl_latest_in.KF_velocity[2],
                    ...
                    mnl_latest_in.GNSS_extra_info[0], mnl_latest_in.GNSS_extra_info[1], mnl_latest_in.GNSS_extra_info[2],
                    mnl_latest_in.GNSS_extra_info[3], mnl_latest_in.GNSS_extra_info[4]);
                mnld2logd_write_mpelog(log_buff, strlen(log_buff));
            }
#endif
            if(mpe_sensor_get_listen_mode() == MPE_START_MODE) {
                mpe_sensor_update_mnl_sec(gps_sec, leap_sec);
...
void *mpe_sensor_thread(void * arg) {
    ...
    mpe_sensor_start(SENSOR_USER_ID_MPE, SENSOR_TYPE_UNCAL_GYR, 20);
    mpe_sensor_start(SENSOR_USER_ID_MPE, SENSOR_TYPE_ACC, 20);
    mpe_sensor_start(SENSOR_USER_ID_MPE, SENSOR_TYPE_MAG, 200);
    mpe_sensor_start(SENSOR_USER_ID_MPE, SENSOR_TYPE_BAR, 20);
    mpe_sensor_start(SENSOR_USER_ID_MPE, SENSOR_TYPE_GYR_TMP, 20);
