...
int FMR_get_cfgs(int idx)
{
    ...
    LOGI("%s successfully. chip: 0x%x, lband: %d, hband: %d, seek_space: %d, max_scan_num: %d\n",
        __FUNCTION__, FMR_chip(idx), FMR_low_band(idx), FMR_high_band(idx),
        FMR_seek_space(idx), FMR_max_scan_num(idx));
    return ret;
}
...
int FMR_set_customized_band_boundary(int low, int high)
{
    fmr_data.cfg_data.low_band_cust = low;
    fmr_data.cfg_data.high_band_cust = high;
    return 0;
}

int FMR_set_region(int band)
{
    fmr_data.cfg_data.band = band;
    if (band == FM_BAND_JAPAN)/* Japan band   76MHz ~ 90MHz */ {
        fmr_data.cfg_data.low_band = 7600;
        fmr_data.cfg_data.high_band = 9600;
    } else if (band == FM_BAND_JAPANW)/* Japan wideband  76MHz ~ 108MHz */ {
        fmr_data.cfg_data.low_band = 7600;
        fmr_data.cfg_data.high_band = 10800;
    } else if (band == FM_BAND_UE)/* US/Europe band  87.5MHz ~ 108MHz (DEFAULT) */ {
        fmr_data.cfg_data.low_band = 8750;
        fmr_data.cfg_data.high_band = 10800;
    }
    LOGE("%s regin:%d, min freq: %d, max_freq:%d\n",
        __func__, band, fmr_data.cfg_data.low_band, fmr_data.cfg_data.high_band);
    return 0;
}

int FMR_set_customized_space(int space)
{
    fmr_data.cfg_data.seek_space_cust = space;
    return 0;

}

int FMR_get_band_boundary(int *low, int *high)
{
    if (fmr_data.cfg_data.band != FM_BAND_SPECIAL) {
        *low = fmr_data.cfg_data.low_band;
        *high = fmr_data.cfg_data.high_band;
    } else {
        *low = fmr_data.cfg_data.low_band_cust;
        *high = fmr_data.cfg_data.high_band_cust;
    }
    return 0;
}
...
int FMR_pwr_up(int idx, int freq, int deemphasis)
{
    int ret = 0;
    ...
    ret = FMR_cbk_tbl(idx).pwr_up(FMR_fd(idx), fmr_data.cfg_data.band, freq, deemphasis);
    if (ret) {
        LOGE("%s failed, [ret=%d]\n", __func__, ret);
    }
...
int FMR_get_af_list(int idx, int16_t **af_list, int *len)
{
    int ret = 0;

    FMR_ASSERT(FMR_cbk_tbl(idx).get_af_list);
    FMR_ASSERT(af_list);
    FMR_ASSERT(len);
    ret = FMR_cbk_tbl(idx).get_af_list(FMR_fd(idx), &fmr_data.rds, af_list, len);
    LOGD("%s, [ret=%d]\n", __func__, ret);
    return ret;
}
...
{
    int ret = 0;
    int low_freq = 0, high_freq = 0;

    FMR_ASSERT(FMR_cbk_tbl(idx).tune);

    FMR_get_band_boundary(&low_freq, &high_freq);
    if ((freq < low_freq) || (freq > high_freq)) {
        LOGE("%s invalid freq: %d\n", __func__, freq);
...
*/
static int FMR_Seek_More(int idx, fm_softmute_tune_t *validfreq,
    fm_u8 seek_dir, fm_u8 step, fm_u16 min_freq, fm_u16 max_freq)
{
    fm_s32 i;
...
return -1 : seek fail
         0: seek success
*/
int FMR_seek_Channel(int idx, int start_freq, int min_freq, int max_freq,
    int band_channel_no, int seek_space, int dir, int *ret_freq, int *rssi_tmp)
{
    fm_s32 i, ret = 0;
    fm_softmute_tune_t cur_freq;
...
int FMR_seek(int idx, int start_freq, int dir, int *ret_freq)
{
    ...
    FMR_get_band_boundary(&low_freq, &high_freq);
    if ((start_freq < low_freq) || (start_freq > high_freq)) {
    ...
    if (fmr_data.cfg_data.band != FM_BAND_SPECIAL) {
        ...
        seek_space = fmr_data.cfg_data.seek_space_cust;
        min_freq = fmr_data.cfg_data.low_band_cust;
        max_freq = fmr_data.cfg_data.high_band_cust;
        band_channel_no = (max_freq - min_freq) / seek_space + 1;
    }
...
int FMR_seek_cust(int idx, int start_freq, int min_freq, int max_freq, int dir, int *ret_freq)
{
    fm_s32 ret = 0, i, j;
    fm_s32 band_channel_no = 0;
    fm_u8 seek_space = 10;
    ...
    if ((start_freq < min_freq) || (start_freq > max_freq)) /*need replace by macro*/ {
        LOGE("%s error start_freq: %d\n", __func__, start_freq);
        return -ERR_INVALID_PARA;
    }
...
int FMR_scan_Channels(int idx, uint16_t *scan_tbl, int *max_cnt,
    fm_s32 band_channel_no, fm_u16 Start_Freq, fm_u8 seek_space, fm_u8 NF_Space)
{
