...
bool mtk_socket_expected_int64_t(char* buff, int* offset, int64_t expected_value, const char* func, int line) {
    ...
    if(value != expected_value) {
        SOCK_LOGE("%s():%d mtk_socket_expected_int64_t() failed, read=[%ld], expected=[%ld]",
            func, line, value, expected_value);
        return false;
    }
    return true;
...
bool safe_sendto(const char* path, const char* buff, int len) {
    ...
    if (fd < 0) {
        SOCK_LOGE("safe_sendto() socket() failed reason=[%s]%d",
            strerror(errno), errno);
        return false;
    }
    ...
    if (fcntl(fd, F_SETFL, flags | O_NONBLOCK) == -1){
        SOCK_LOGE("fcntl failed reason=[%s]%d",
                    strerror(errno), errno);
    ...
    while ((ret = sendto(fd, buff, len, 0,
        ...
        SOCK_LOGE("safe_sendto() sendto() failed path=[%s] ret=%d reason=[%s]%d",
...
int socket_bind_udp(const char* path) {
    ...
    if (fd < 0) {
        SOCK_LOGE("socket_bind_udp() socket() failed reason=[%s]%d",
            strerror(errno), errno);
        return -1;
    }
    SOCK_LOGD("fd=%d,path=%s\n", fd, path);
    ...
    if (bind(fd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
        SOCK_LOGE("socket_bind_udp() bind() failed path=[%s] reason=[%s]%d",
...
int epoll_add_fd(int epfd, int fd) {
    ...
    if (epoll_ctl(epfd, EPOLL_CTL_ADD, fd, &ev) == -1) {
        SOCK_LOGE("epoll_add_fd() epoll_ctl() failed reason=[%s]%d epfd=%d fd=%d",
