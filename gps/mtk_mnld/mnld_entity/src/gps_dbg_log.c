...
char gps_debuglog_file_name[GPS_DEBUG_LOG_FILE_NAME_MAX_LEN] = "/mnt/sdcard/mtklog/connsyslog/gpshost/GPS_HOST";
char storagePath_mtklogger_set[GPS_DEBUG_LOG_FILE_NAME_MAX_LEN] = "/mnt/sdcard/mtklog/connsyslog/gpshost/";
char storagePath[GPS_DEBUG_LOG_FILE_NAME_MAX_LEN] = "/mnt/sdcard/mtklog/connsyslog/gpshost/";
...
//  data
#define PINGPANG_BUFFER_SIZE         (15*1024)
...
static int get_mtklog_path(char *logpath) {
    ...
    if (len <= GPS_DEBUG_LOG_FILE_NAME_MAX_LEN - strlen(PATH_SUFFIX)) {
        sprintf(logpath, "%smtklog/connsyslog/gpshost/", temp);
...
int mtklogger2mnl_hdlr(int server_fd, mtk_socket_fd* client_fd) {
    ...
    ret = mtk_socket_read(server_fd, buff_msg, sizeof(buff_msg));
    if(ret == -1) {
...
static bool gps_dbg_log_pingpang_write(ping_pang_buffer* pingpang) {
    ...
    if ((pingpang->buffer1_state == READABLE)\
        ...
        ret = mnld2logd_write_gpslog(pingpang->start_address_buffer1, pingpang->buffer1_lenth_to_write);
        memset(pingpang->start_address_buffer1, 0x0, PINGPANG_BUFFER_SIZE);
        pingpang->buffer1_lenth_to_write = 0;
        ...
        ret = mnld2logd_write_gpslog(pingpang->start_address_buffer2, pingpang->buffer2_lenth_to_write);
        memset(pingpang->start_address_buffer2, 0x0, PINGPANG_BUFFER_SIZE);
        pingpang->buffer2_lenth_to_write = 0;
...
static bool gps_dbg_log_pingpang_flush(ping_pang_buffer * pingpang) {
    ...
    if ((pingpang->buffer1_state == WRITING) && (pingpang->buffer2_state != WRITING)) {
        ...
        ret = mnld2logd_write_gpslog(pingpang->start_address_buffer1, tmp_next_write - pingpang->start_address_buffer1);
    } else if ((pingpang->buffer2_state == WRITING) && (pingpang->buffer1_state != WRITING)) {
        if (pingpang->buffer1_state == READABLE) {
        ...
        ret = mnld2logd_write_gpslog(pingpang->start_address_buffer2, tmp_next_write - pingpang->start_address_buffer2);
    } else {
