...
#define MTKLOG_DIR            "/sdcard/mtklog/"
#define CONNSYSLOG_DIR        "/sdcard/mtklog/connsyslog/"
#define COMBO_T32_DIR         "/sdcard/mtklog/connsyslog/connsysdump/"
...
int stp_dump_ctrl_iface_trace_logger_init(struct trace_iface_priv *tr_priv) {
    ...
    t_counter = 0;
    while (120 > t_counter) {
        if (mkdir(CONNSYSLOG_DIR, 00777) < 0) {
            if (errno == EEXIST) {
                stp_printf(MSG_INFO, "Using existing control interface directory.\n");
                break;
            } else {
                stp_printf(MSG_ERROR, "mkdir (%s) failed:(%s)\n", CONNSYSLOG_DIR, strerror(errno));
                perror("mkdir[CONNSYSLOG_DIR]");
                os_sleep(0, 500000);
                t_counter++;
            }
        } else {
